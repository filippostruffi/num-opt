...
def build_model(task: Any, dataset: Any) -> nn.Module:
    ...
    word_vocab = getattr(dataset, "word_vocab", getattr(base, "word_vocab", None))
    if word_vocab is None:
        raise ValueError("word_vocab not found on dataset")
    num_tags = getattr(dataset, "num_tags", getattr(base, "num_tags", None))
    if num_tags is None:
        raise ValueError("num_tags not found on dataset")
    # Expect char_vocab present from dataset builder
    char_vocab = getattr(dataset, "char_vocab", getattr(base, "char_vocab", None))
    num_chars = len(char_vocab) if isinstance(char_vocab, dict) else 128
    # Expose vocabs for task usage if needed
    task.runtime_objects["word_vocab"] = word_vocab
    if isinstance(char_vocab, dict):
        task.runtime_objects["char_vocab"] = char_vocab
    return LSTMCRFCharCNNNER(vocab_size=len(word_vocab), num_chars=num_chars, num_tags=num_tags)
