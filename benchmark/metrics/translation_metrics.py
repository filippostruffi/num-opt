...
def simple_bleu(candidate: List[str], reference: List[str], max_n: int = 4) -> float:
    # Minimal BLEU (no brevity penalty smoothing)
    import math
    ...
    for n in range(1, max_n + 1):
        ...
        prec = match / total
        score += weights[n - 1] * (math.log(prec + 1e-9))
    bp = 1.0  # ignore brevity penalty for simplicity
    return float(bp * (2.718281828459045 ** score))
...
class TranslationMetrics:
    ...
    def compute(self, acc: Dict[str, Any], tgt_vocab: Optional[Dict[str, int]] = None) -> Dict[str, Any]:
        ...
        for cand_ids, ref_ids in zip(acc["cands"], acc["refs"]):
            ...
            # METEOR and TER simplified proxies
            meteors.append(rouge_l(cand, ref))
            # TER proxy: 1 - ROUGE_L
            ters.append(1.0 - rouges[-1])
        if len(bleus) == 0:
            return {"bleu": 0.0, "meteor": 0.0, "rouge": 0.0, "ter": 1.0}
        return {
