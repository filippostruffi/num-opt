...
def build_conll2003(max_samples: Optional[int], data_root: str) -> Tuple[object, object]:
    cache_dir = os.path.join(data_root, "conll2003") if data_root is not None else None

    ds = load_dataset("lhoestq/conll2003", cache_dir=cache_dir)

    train_split = ds["train"]
    val_split = ds["validation"] if "validation" in ds else ds["test"]

    # If you want to limit BEFORE wrapping:
    train_split, val_split = maybe_limit_split(train_split, val_split, max_samples)

    if isinstance(train_split, Subset):
        base_ds = train_split.dataset
        indices = train_split.indices
        tokens_train = [base_ds[int(i)]["tokens"] for i in indices]
    else:
        tokens_train = train_split["tokens"]
    ...
    ner_feat = ds["train"].features["ner_tags"]
    if hasattr(ner_feat, "feature") and hasattr(ner_feat.feature, "num_classes"):
        num_tags = int(ner_feat.feature.num_classes)
    elif hasattr(ner_feat, "feature") and hasattr(ner_feat.feature, "names"):
        num_tags = len(ner_feat.feature.names)
    elif hasattr(ner_feat, "num_classes"):
        num_tags = int(ner_feat.num_classes)
    elif hasattr(ner_feat, "names"):
        num_tags = len(ner_feat.names)
    else:
        # Fallback: derive from label ids present in the (possibly limited) split
        if isinstance(train_split, Subset):
            base_ds = train_split.dataset
            indices = train_split.indices
            all_tags = []
            for i in indices:
                sample = base_ds[int(i)]
                all_tags.extend(sample["ner_tags"])
        else:
            all_tags = [t for seq in ds["train"]["ner_tags"] for t in seq]
        num_tags = int(max(all_tags)) + 1 if all_tags else 10
